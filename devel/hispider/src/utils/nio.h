...
#define NIO_CHECK(ptr) ( (PF(ptr) == NULL  || (PF(ptr)->fd <= 0  \
                && (PF(ptr)->fd = open(PF(ptr)->path, O_CREAT|O_RDWR, 0644)) <= 0 ) ) ? -1 : 0)
...
#define NIO_SREAD(ptr, ps, n, off) ( (NIO_CHECK(ptr) == 0) ?    \
            ( (flock(PF(ptr)->fd, LOCK_EX|LOCK_NB) == 0) ?  \
              ( (lseek(PF(ptr)->fd, off, SEEK_SET) >= 0) ?  \
                (read(PF(ptr)->fd, ps, n) | flock(PF(ptr)->fd, LOCK_UN)) \
                  : (-1 | flock(PF(ptr)->fd, LOCK_UN))) : -1) : -1)
#define NIO_SWRITE(ptr, ps, n, off) ( (NIO_CHECK(ptr) == 0) ?    \
            ( (flock(PF(ptr)->fd, LOCK_EX|LOCK_NB) == 0) ?  \
              ( (lseek(PF(ptr)->fd, off, SEEK_SET) >= 0) ?  \
                (write(PF(ptr)->fd, ps, n) | flock(PF(ptr)->fd, LOCK_UN)) \
                  : (-1 | flock(PF(ptr)->fd, LOCK_UN))) : -1) : -1)
#define NIO_APPEND(ptr, ps, n) ( (NIO_CHECK(ptr) == 0) ?    \
            ( (flock(PF(ptr)->fd, LOCK_EX|LOCK_NB) == 0) ?  \
              ( (lseek(PF(ptr)->fd, 0, SEEK_END) >= 0) ?  \
                (write(PF(ptr)->fd, ps, n) | flock(PF(ptr)->fd, LOCK_UN)) \
                  : (-1 | flock(PF(ptr)->fd, LOCK_UN))) : -1) : -1)
